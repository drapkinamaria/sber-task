...
export const charactersFetchSucceeded = createAction<FilmWithCharacters[]>('CHARACTERS_FETCH_SUCCEEDED');
export const charactersFetchFailed = createAction<string>('CHARACTERS_FETCH_FAILED');
...
export const charactersReducer = createReducer(initialState, (builder) => {
    builder
        .addCase(charactersFetchRequested, (state, action: PayloadAction<string[], string, FetchCharactersMeta>) => {
            state.loading = true;
            state.error = null;
            state.activePlanet = action.meta.planetName;
        })
        .addCase(charactersFetchSucceeded, (state, action: PayloadAction<FilmWithCharacters[]>) => {
