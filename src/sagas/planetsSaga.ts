...
import {
  planetsFetchFailed,
  planetsFetchRequested,
  planetsFetchSucceeded,
} from "../store/planetsReducer";
import { PayloadAction } from "@reduxjs/toolkit";

const fetchPlanetsFromApi = (pageNumber: number): Promise<PlanetsRequest> =>
  fetch(`https://swapi.dev/api/planets/?page=${pageNumber}`).then(
    (response) => {
...
function* fetchPlanetsWorker(
  action: PayloadAction<number>,
): Generator<CallEffect<PlanetsRequest> | PutEffect, void, PlanetsRequest> {
  try {
    const data: PlanetsRequest = yield call(
      fetchPlanetsFromApi,
      action.payload,
    );
