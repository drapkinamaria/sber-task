...
export function MainPage() {
    ...
    function handlePagination(next: boolean) {
        ...
        setPageNumber(prevPageNumber => next ? prevPageNumber + 1 : prevPageNumber - 1);
    }

    return (
        <div className="container custom-container">
            <h1>Planets</h1>
            {loadingPlanets ? <LoadingSpinner /> : (
                <>
                    {errorPlanets && <p className="alert alert-danger" role="alert">Error fetching planets: {errorPlanets}</p>}
                    <PlanetList planets={planets} handleClickPlanetButton={handleClickPlanetButton} />
                </>
            )}
